  ...
  if (isError) {
    return (
      <Alert severity="error">
        {(error as any)?.response?.data?.message || 'Não foi possível carregar as questões.'}
      </Alert>
    );
  }
    ...
    <Box sx={{ p: { xs: 2, md: 4 }, backgroundColor: '#f6f8fb', minHeight: '100%' }}>
      <Stack spacing={3}>
        <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', sm: 'center' }} spacing={2}>
          <Box>
            <Typography variant="h4" fontWeight={700}>Questões</Typography>
            <Typography color="text.secondary" mt={0.5}>Gerencie a listagem e visualize rapidamente as questões cadastradas.</Typography>
          </Box>

          <Button
            variant="contained"
            onClick={() => router.push('/questoes/nova')}
            sx={{ borderRadius: 2.5, px: 2.5, py: 1.1, fontWeight: 700, textTransform: 'none', boxShadow: 'none' }}
          >
            Nova questão
          </Button>
        </Stack>

        <Paper elevation={0} sx={{ borderRadius: 4, border: '1px solid #e8edf3', overflow: 'hidden', bgcolor: '#fff' }}>
          <Box sx={{ p: 3, borderBottom: '1px solid #eef2f7' }}>
            <Typography variant="h6" fontWeight={700}>Lista de questões</Typography>
            <Typography variant="body2" color="text.secondary">Exibindo banca, disciplina, assunto, instituição, ano e indicação de texto de apoio.</Typography>
          </Box>
                    ...
                    <TableCell align="right">
                      <Stack direction="row" spacing={1} justifyContent="flex-end">
                        <Tooltip title="Ver questão completa">
                          <IconButton color="primary" onClick={() => abrirQuestao(q)}>
                        ...
                        <Tooltip title={isAdmin ? 'Excluir questão' : 'Apenas administradores podem excluir questões'}>
                          <span>
                            <IconButton color="error" onClick={() => handleExcluir(q.idQuestion)} disabled={!isAdmin || excluirQuestao.isPending}>
                              <DeleteOutlineIcon />
                            </IconButton>
                          </span>
                        </Tooltip>
      ...
      <Dialog open={Boolean(questaoSelecionada)} onClose={() => setQuestaoSelecionada(null)} fullWidth maxWidth="md" PaperProps={{ sx: { borderRadius: 4, overflow: 'hidden', boxShadow: '0 24px 80px rgba(15, 23, 42, 0.18)', backgroundColor: '#f8fafc' } }}>
        {questaoSelecionada && (
          <>
            <DialogContent sx={{ p: 3, pt: 4, backgroundColor: '#f8fafc' }}>
              <Paper elevation={0} sx={{ borderRadius: 3, border: '1px solid #e5e7eb', overflow: 'hidden', backgroundColor: '#fff' }}>
                <Box sx={{ p: 3 }}>
                  <Stack spacing={2.5}>
                    <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }}>
                      ...
                      <Typography variant="body2" sx={{ color: '#64748b', fontWeight: 600 }}>{questaoSelecionada.disciplina} {'›'} {questaoSelecionada.assunto}</Typography>
                    </Stack>
                    ...
                    {questaoSelecionada.textoApoioConteudo && (
                      <Paper elevation={0} sx={{ p: 2.5, borderRadius: 3, border: '1px solid #dbeafe', backgroundColor: '#eff6ff' }}>
                        <Stack spacing={1}>
                          <Typography variant="subtitle2" fontWeight={800} color="#1e40af">
                            {questaoSelecionada.textoApoioTitulo || 'Texto de apoio'}
                          </Typography>
                    ...
                    <Typography variant="body1" sx={{ color: '#475569', lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>{questaoSelecionada.enunciado}</Typography>

                    <Paper elevation={0} sx={{ p: 2.5, borderRadius: 3, border: '1px solid #e5e7eb', backgroundColor: '#fcfcfd' }}>
                      <Typography variant="body1" sx={{ fontSize: '1.05rem', fontWeight: 600, color: '#374151', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{questaoSelecionada.questao}</Typography>
                    </Paper>
                          ...
                          return (
                            <Box
                              ...
                              sx={{
                                ...
                                '&:hover': respondeu ? {} : { borderColor: '#2563eb', backgroundColor: '#f1f5f9' },
                              }}
                            >
                              ...
                              <Typography variant="body1" sx={{ fontWeight: isCorreta && respondeu ? 700 : 600, color: '#374151', lineHeight: 1.7 }}>{alternativa}</Typography>
                            </Box>
                          );
                        })}
                    </Stack>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} alignItems={{ xs: 'flex-start', sm: 'center' }}>
                      <Button variant="contained" disableElevation onClick={() => { if (respostaSelecionada) setRespondeu(true); }} sx={{ textTransform: 'none', fontWeight: 700, borderRadius: 2, px: 2.5, backgroundColor: '#2563eb' }}>Responder</Button>
                      <Button variant="text" onClick={() => { setRespostaSelecionada(null); setRespondeu(false); }} sx={{ textTransform: 'none', fontWeight: 700, color: '#64748b' }}>Limpar</Button>
                    </Stack>
                  </Stack>
                </Box>
              </Paper>
            ...
            <DialogActions sx={{ px: 3, py: 2, borderTop: '1px solid #e5e7eb', backgroundColor: '#fff', justifyContent: 'flex-end' }}>
              <Button variant="contained" onClick={() => setQuestaoSelecionada(null)} sx={{ borderRadius: 2, px: 2.5, textTransform: 'none', fontWeight: 700, boxShadow: 'none' }}>Fechar</Button>
            </DialogActions>
          </>
        )}
      </Dialog>
